...
export async function createMetadata(
  ...
  console.log('metadataAccount Data --->', data);
  const value = new CreateMetadataArgs({ data, isMutable: true });
  console.log('CreateMetadataArgs value --->', value);
  const txnData = Buffer.from(serialize(METADATA_SCHEMA, value));
...
export async function updatePrimarySaleHappenedViaToken(
  ...
  console.log(
    'updatePrimarySaleHappenedViaToken keys-------->',
    keys.map(e => e.pubkey.toBase58()),
  );
}

export async function asyncWSol(
  owner: StringPublicKey,
  instructions: TransactionInstruction[],
) {
  ...
  instructions.push(
    new TransactionInstruction({
      keys,
      programId: toPublicKey(tokenProgramId),
      data,
    }),
  );
  console.log(
